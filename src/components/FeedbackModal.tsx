...
export default function FeedbackModal({ onClose }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const entry = { name: name.trim() || 'Anonymous', email: email.trim(), message: message.trim(), timestamp: Date.now() }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Title bar */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '14px 20px 12px', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          ...
          <span style={{ marginLeft: 8, fontWeight: 600, fontSize: 14, color: 'rgba(255,255,255,0.88)' }}>Send Feedback</span>
        </div>
        ...
        {sent ? (
          ...
          <form onSubmit={handleSubmit} style={{ padding: '22px 28px 28px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <div style={{ flex: 1 }}>
                <label style={{ display: 'block', fontSize: 11.5, color: 'rgba(255,255,255,0.42)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Name (optional)</label>
                <input value={name} onChange={e => setName(e.target.value)} placeholder="Your name" style={inputStyle}
                  onFocus={e => (e.target.style.borderColor = 'rgba(99,160,255,0.55)')}
                  onBlur={e => (e.target.style.borderColor = 'rgba(255,255,255,0.14)')} />
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ display: 'block', fontSize: 11.5, color: 'rgba(255,255,255,0.42)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Email (optional)</label>
            ...
            <div>
              <label style={{ display: 'block', fontSize: 11.5, color: 'rgba(255,255,255,0.42)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Message *</label>
              <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="Write your feedback here..." rows={4} required
            ...
            <button type="submit" disabled={!message.trim()} style={{
              ...
              fontSize: 13.5, fontWeight: 600, cursor: message.trim() ? 'pointer' : 'not-allowed',
