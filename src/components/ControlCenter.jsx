  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 12, padding: wide ? '10px 14px' : '10px 12px',
    ...
    >
      <span style={{ fontSize: 18, lineHeight: 1, filter: active ? 'none' : 'brightness(2)' }}>
        {icon}
      </span>
      <span style={{
        fontSize: 10.5, fontWeight: 600,
        color: active ? 'rgba(0,0,0,0.85)' : 'rgba(255,255,255,0.75)',
        lineHeight: 1.2,
      }}>
        {label}
      </span>
...
function Slider({ value, onChange, icon, color = '#63a0ff' }) {
  ...
  useEffect(() => {
    const onMove = (e) => { if (dragging.current) onChange(getValueFromEvent(e)) }
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      >
        ...
        <div style={{
          ...
          width: `${value * 100}%`, borderRadius: 99,
          background: color, transition: dragging.current ? 'none' : 'width 0.05s',
          pointerEvents: 'none',
        }} />
        {/* Thumb */}
        <div style={{
...
function ProgressBar({ current, duration, onSeek }) {
  ...
  useEffect(() => {
    const onMove = (e) => { if (dragging.current) onSeek(getTime(e)) }
    const onUp   = () => { dragging.current = false }
    window.addEventListener('mousemove', onMove)
    window.addEventListener('mouseup', onUp)
    return () => { window.removeEventListener('mousemove', onMove); window.removeEventListener('mouseup', onUp) }
...
/* ── main control center ────────────────────────────────────────── */
export default function ControlCenter({ show, onClose, audioControls, theme, onThemeChange }) {
  ...
  useEffect(() => {
    if (!show) return
    const handler = (e) => {
      if (!e.target.closest('[data-cc]')) onClose()
    }
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
          ...
          <div style={{
            ...
            background: 'linear-gradient(135deg,#1e3a5f,#2d1b5a)',
          ...
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" style={{ opacity: 0.4, flexShrink: 0 }}>
            <path d="M5 17H3a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h18a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2h-2" stroke="white" strokeWidth="2"/>
            <polygon points="12,15 17,21 7,21" fill="white" opacity="0.6"/>
          </svg>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 22, marginTop: 10 }}>
          ...
          <button
            onClick={() => seek(0)}
            style={{ background:'none', border:'none', cursor:'pointer', padding:0, color:'rgba(255,255,255,0.75)', fontSize:0, lineHeight:1 }}
          >
          ...
          <button
            ...
            style={{
              ...
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              color: '#111', fontSize: 0, lineHeight: 1,
              boxShadow: '0 2px 8px rgba(0,0,0,0.4)',
              transition: 'transform 0.1s',
            }}
            onMouseDown={e => e.currentTarget.style.transform = 'scale(0.9)'}
            onMouseUp={e => e.currentTarget.style.transform = 'scale(1)'}
            title={canPlay ? '' : 'Audio format not supported — convert to MP3'}
          >
            {isPlaying ? (
      ...
      <Slider
        value={volume}
        onChange={setVolume}
        icon={volume === 0 ? '🔇' : volume < 0.4 ? '🔈' : volume < 0.8 ? '🔉' : '🔊'}
        color="#a78bfa"
      />
      ...
      {!canPlay && (
        <div style={{
          ...
          fontSize: 10.5, color: 'rgba(254,188,46,0.9)', lineHeight: 1.5,
        }}>
          ⚠️ <strong>.mgg</strong> is not a browser-supported format. Rename to <strong>.ogg</strong> if it's
          an OGG file, or convert to <strong>.mp3 / .m4a</strong>.
        </div>
      )}
    </div>
  )
